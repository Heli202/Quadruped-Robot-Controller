...
class Quadruped:
    ...
    def get_all_positions(self): # These two will have to use statemanager receive command, maybe send a command like "what is the values" and it sends that back?
        ...
        for leg in self.quadruped.legs:
            for servo in leg.servos:
                if servo.name in state:
                    ...
                    print(f"No value found for {servo.name} in the loaded state")
